\chapter{Upscaling for systems with identical components: numerical integration with constant correlation}\label{Section_2.4.3}
...
The value of $\beta$ is considered to be known, i.e.\ it is determined by the probabilistic computation techniques as described in \Aref{sec:ReliabilityTechniques}.
...
\includegraphics*[width=5.93in, height=1.58in, keepaspectratio=false]{Pictures/variation_in_time_and_space}
...
\begin{figure}[H]\centering
\includegraphics*[width=4.45in, height=3.34in, keepaspectratio=false]{Pictures/exponentional_distribution}
...
\Note{in the \probLib, the outcrossing method is applied in time as well as in space.
First, the probabilities of failure of the smallest ''components'' are computed with the probabilistic techniques for single components as described in \Aref{sec:ReliabilityTechniques}.
...
Note that the component for which  $P_F(0)$ is computed in the \probLib has a ''width'' equal to the assumed breach width.
...
In the \probLib this correlation is described with the following model:
\begin{align}
\rho \left(\Delta y\right)=\rho _{x} +(1-\rho _{x} )\exp \left[-\left(\frac{\Delta y^{2} }{d_{x}^{2} } \right)\right]\label{ZEqnNum807806} 
\end{align}
where $\rho$ is the correlation between two locations within the segment ($\rho \geqslant 0$), $\Delta_y$ is the distance between these two locations, $\rho_x$ is the residual correlation length of variable $X$ and $d_x$ is the spatial correlation length of variable $X$.
...
\begin{figure}[H]\centering
\includegraphics*[width=4.09in, height=3.06in, keepaspectratio=false]{Pictures/autocorrelation_function}
...
For this purpose the probabilistic techniques for a single component are used (see \Aref{sec:ReliabilityTechniques}).
...
\end{align}
where $\beta$ is the reliability index as derived in the probabilistic computation for a cross-section (see \Aref{sec:ReliabilityTechniques}).
...
\end{align}
where $\rho$ is the correlation between two locations within the segment and $\Delta_y$ is the distance between two locations.
Using the theorem of total probability, the failure probability of the flood defence segment can be described as follows:
\begin{align}
...
The combination of equations \eqref{ZEqnNum551150} and \eqref{ZEqnNum591781} provide the probability of failure for a flood defence segment.
...
Equation \eqref{ZEqnNum591781} can be evauated with high accuracy using for example numerical integration.

\Note{in formula \ref{eq:sm3}  the width of the mechanism is not taken into account.
In the \probLib the width of the mechanism is taken equal to $\Delta L$.
...
For values of $\rho_z = 0$ the Hohenbichler method is used in the \probLib in combination with the outcrossing approach.}
