...
namespace Deltares
{
    namespace ProbLibTests
    {
        ...
        void upscale_tests::upscaleInTimeTests2()
        {
            ...
            auto ref = alphaBeta(1.0 - pow(p, nrElements), 'q', element.getAlpha());
        ...
        void upscale_tests::upscaleToLargestBlockTests3()
        {
            ...
            const auto ORG = alphaBeta(beta.getu(), alpha); // Copy of the original beta and alpha values
            const auto durationORG = duration;  // Copy of the original durations
            const auto maxduration = duration.maxval();

            const double p = beta.getp();
            auto ref = alphaBeta(1.0 - pow(p, 10), 'q', ORG.getAlpha());
