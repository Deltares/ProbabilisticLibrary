...
class DesignPoint(FrozenObject):
	...
	def reliability_results(self) -> list[ReliabilityResult]:
		"""List of convergence reports during the reliability analysis. Depends on the setting 'save_convergence'
	...
	def plot_convergence(self):
        ...
        Only available when 'save_convergence' was set in the settings"""
...
class Alpha(FrozenObject):
    ...
    The contribution consists of two indicators, the influnce of the uncertainty of the stochastic
...
class FragilityCurve(FrozenObject):
    ...
    A fragility curve is similar to a stochast with distribution type cdf_curve. A fragility curve
    has an additional method 'integrate' to calculate the reliability, when the distribution of the
    x-values is know."""
...
class CombineSettings(FrozenObject):
    ...
    These settings are used by the CombineProject"""
...
class ExcludingCombineSettings(FrozenObject):
    ...
    These settings are used by the ExclusiveCombineProject"""
