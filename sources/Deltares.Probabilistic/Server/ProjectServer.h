...
namespace Deltares
{
    namespace Server
    {
        ...
        public:
            ...
            ProjectServer(std::shared_ptr<BaseHandler> handler, SharedDataBetweenHandles* sd)
            {
                std::shared_ptr<BaseHandler> defaultHandler = std::make_shared<ProjectHandler>(sd);
                this->AddHandler(defaultHandler);
                this->AddHandler(handler);
            }
