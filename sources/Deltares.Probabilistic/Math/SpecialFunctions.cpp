...
namespace Deltares
{
    namespace Numeric
    {
        ...
        {
            double betaIncomplete = getBetaIncomplete(a, b, x);
            double beta = getBeta(a, b);
            return betaIncomplete / beta;
        }
        ...
        double SpecialFunctions::getBetaIncomplete(double a, double b, double x)
        {
            ...
            if (x > (a + 1.0) / (a + b + 2.0)) {
                return (1.0 - getBetaIncomplete(b, a, 1.0 - x)); /*Use the fact that beta is symmetrical.*/
            }
            ...
            return std::nan(""); /*Needed more loops, did not converge.*/
        }
    }
}
