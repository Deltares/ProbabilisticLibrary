...
namespace Deltares::Reliability
{
    ...
    public:
        sphereTasks()
            : maxSteps(5), maxAngle(std::numbers::pi / 2.0) {}

        explicit sphereTasks(const size_t mxSteps, const bool allQuadrants)
            : maxSteps(mxSteps), maxAngle(allQuadrants ? 2.0 * std::numbers::pi : std::numbers::pi / 2.0) {}
