...
\section{Model return type}
\label{sec:ModelReturnType}

Until so far we assumed that a model return a z-value, which should be interpreted as failing if smaller than zero or non failing otherwise. Some reliability methods support the model return a probability of failure or a reliability index.

The model result $z$ is converted to an addition of failure as follows, depending on the model result type:

\begin{align} 
	\label{eq:SampleAddition}
	p_{\text{i}}\left(z\right) = \left\{\begin{array}{ll}\text{z-value} & z < 0 ? 1 : 0\\
		\text{probability of failure} & z \\
		\text{reliability index} & \Phi^{-1}\left(z\right) \\
	\end{array}	
	\right.
\end{align}
...
For variables $i$, $j$, $k$ the probability of failure is calculated as follows:

\begin{align} 
	\label{eq:NumericalIntegration}
	p_{\text{f}} = \sum_{\text{i}} \sum_{\text{j}} \sum_{\text{k}} w_{\text{i}} \cdot w_{\text{j}} \cdot w_{\text{k}} \cdot p_{\text{i,j,k}}
\end{align}

where the weight $w_{\text{i}}$ is defined as

\begin{align} 
	\label{eq:NumericalIntegrationWeight}
	w_{\text{i}} = \Phi^{-1}\left(u_{\text{i+1}}\right) - \Phi^{-1}\left(u_{\text{i}}\right)
\end{align}

and

\begin{longtable}{p{16mm}p{\textwidth-24pt-16mm}}
	$i$, $j$, $k$ & are indices of variables ranging over the complete variable space. \\  
	$p_{\text{f}}$ & is the probability of failure \\  
	$p_{\text{i,j,k}}$ & is the individual addition of a realization, see \autoref{eq:SampleAddition} \\ 
	$u_{\text{i}}$ & is the u-value corresponding with a realization of variable $i$. The index $i$ ranges over such values that $u_{\text{i}}$ ranges over $-8$ to $8$. Beyond $u=8$ numerical instability occurs, but $u=8$ corresponds to a probability of approximately $6 \cdot 10^{−16}$, which is orders of magnitude smaller than the probabilities of typical scenarios. \\
	$\Phi$ & is the CDF of the standard normal distribution (see \autoref{eq:StandardNormalCDF}) \\	
\end{longtable}
...
Crude Monte Carlo is the standard Monte Carlo simulation method. Random realizations are generated in proportion to their probability density. Each realization $i$ leads to an addition $p_{\text{i}}$ (\autoref{eq:SampleAddition}) to the total failure. 
	
The number of realizations that result in failure is counted, as well as the number that do not lead to failure. The probability of failure is then calculated as follows:

\begin{align} 
\label{eq:MonteCarlo}
p_{\text{f}} = \frac{\sum_{\text{i}} p_{\text{i}}}{N}
\end{align}

where:
\begin{longtable}{p{16mm}p{\textwidth-24pt-16mm}}  
	$p_{\text{f}}$ & is the probability of failure \\  
	$p_{\text{i}}$ & is the individual addition of a realization, see \autoref{eq:SampleAddition} \\ 
...
The Monte Carlo simulation also leads to a standard deviation of the probability of failure. This is the standard deviation of contributions $p_{\text{i}}$ of all samples, calculated as follows:

\begin{align} 
	\label{eq:MonteCarloStandardDeviation}
	\sigma_p = \sqrt{\frac{\sum_i \left(p_{\text{i}} - p_{\text{f}}\right)^2}{N}}
\end{align}

In case the model result is either failing or not failing, meaning $p_{\text{i}}$ is always 1 or 0, the standard deviation is equal to:

\begin{align} 
	\label{eq:MonteCarloStandardDeviationZValue}
	\sigma_{p \text{, z-value}} = \sqrt{p_{\text{f}} \cdot \left(1 - p_{\text{f}} \right) }
\end{align}

The next step is computing the standard error $SE$. This is defined as follows:

\begin{align} 
	\label{eq:MonteCarloStandardError}
	SE = \frac{\sigma_p}{\sqrt{N}}
\end{align}

The coefficient of variation $\varepsilon$ is expressed as the relative standard error, relative to the probability of failure or the probability of non failure:

\begin{align} 
	\label{eq:MonteCarloCoeffiecientOfVariation}
	\varepsilon = \frac{SE}{\text{min}\left(p_{\text{f}}, 1 - p_{\text{f}}\right)   }
\end{align}

The user can set the maximum variation coefficient $\varepsilon_{\text{max}}$. The Monte Carlo simulation will stop when $\varepsilon \leq \varepsilon_{\text{max}}$ (and limited to the minimum and maximum number of realizations).
...
\label{sec:MCConfidence}

The confidence level $z$ is always $1$ in the \probLib and can not be set by the user. The confidence level should be applied to $\sigma_p$, defined in \autoref{eq:MonteCarloStandardDeviation}, giving:

\begin{align} 
	\label{eq:MonteCarloStandardDeviationConfidence}
	\sigma_{p\text{, confidence}} = z \cdot \sigma_p
\end{align}
...
p_{\text{failure}} = \frac{\sum_{\text{i}} p_{\text{i}} \cdot W_{\text{i}}}{\sum_{\text{i}} W_{\text{i}}}
\end{align}

where:

\begin{longtable}{p{16mm}p{\textwidth-24pt-16mm}}  
	$p_{\text{i}}$ & is the failure addition of a single realization, see \autoref{eq:SampleAddition} \\  
	$p_{\text{failure}}$ & is the resulting probability of failure
\end{longtable}To avoid numerical issues and because:
...
p_{\text{failure}} = \frac{\sum_{\text{i}} p_{\text{i}} \cdot W_{\text{i}}}{N}
\end{align}
...
Upscaling is distinguished from the more generic system combine methods because the components being identical allows for a more convenient and efficient combination procedure.
