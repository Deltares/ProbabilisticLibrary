...
namespace Deltares {
    namespace ProbLibCore {
        ...
        george_marsaglia::george_marsaglia(const int ij, const int kl) : s1(ij), s2(kl)
        {
            init(ij, kl);
        }

        void george_marsaglia::restart()
        {
            init(s1, s2);
        }

        void george_marsaglia::init(const int ij, const int kl)
        {
            //
            ...
            if (ij < 0 || ij > 31328)
            {
                throw probLibException("Random seed parameter 1 outside of valid range (0 ... 31328)", ij);
            }
            if (kl < 0 || kl > 30081)
            {
                throw probLibException("Random seed parameter 2 outside of valid range (0 ... 30081)", kl);
            }
            //
            // Initialize the parameters of the random number generator
            //
            int i = mod(ij / 177, 177) + 2;  // first parameter in the random number generator
            int j = mod(ij, 177) + 2;  // second param
            int k = mod(kl / 169, 178) + 1;  // third param
            int l = mod(kl, 169);      // fourth param
            //
        ...
        double george_marsaglia::get_uniform()
        {
            //
